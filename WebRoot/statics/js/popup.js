...
function popLogin(type){
    layui.use('layer', function(){ //独立版的layer无需执行这一句
        ...
        layer.open({
            ...
            '</div>'+
            '<div class="shouji">'+
            ' <input type="text" id="renter_mobile" name="renter_mobile_login" class="phone phone-new" placeholder="手机号" maxlength="11"/>'+
...
function changeVerifyImagePop(){
    var nowMobile= $('input[name="renter_mobile_login"]').val();
    var tdata = new Date();
...
function sendVerifyCodePop(pop_index,type){
    ...
    var renter_mobile = $('input[name="renter_mobile_login"]').val();
    // isRunning = true;
    if(image_code_name == ''){
...
function addPopLogin(index){
    renter_mobile = $('.shouji input[name="renter_mobile_login"]').val();
...
function yuyueConfirm(obj){
    if(obj.innerHTML=='查看预约'){

        if(isDelay=='1'){
            window.location.href="/menu?p=2";
            return;
        }else{
            window.location.href="/menu?p=0";
            return;
        }
    }
    var objP = $(obj).parent();
    var renter_mobile = document.forms['info_form']['renter_mobile'].value;
    ...
    layui.use('layer', function(){ //独立版的layer无需执行这一句
        ...
        layer.open({
            ...
            '<div class="seeRoom-pop">' +
                '<h2>申请预约看房</h2>' +
                '<p style="margin-top:20px; color:#FF5C34";>*请填写以下信息,巴乐兔管家会尽快联系你</p>'+
                '<div class="login-inp" >' +
                '    <div class="unit-inp" style="margin-top:10px;">' +
                ...
                '       <input type="text" name="renter_mobile" placeholder="手机号" value="'+renter_mobile+'" id="mobile_renter" maxlength="11">' +
                '       <p id="renter_mobile-prompt-text" style="display: none ;color: red" class="prompt-text-yuyue mobile_text">请您填写正确有效的手机号</p>' +
                '    </div>' +
                '    <div class="unit-inp choose_time">' +
                // '       <img src="<?=G_DOMAIN_JS?>/static/pc/images/jiant_down_mac.png">         '+
                '       <input type="text"  placeholder="选择接听回电时间" value="尽快回电" id="choonse_callback_time" readonly>' +
                '       <div class="choonse_time_content">'+
                '           <div class="choonse_time_content_day">'+
                '               <ul>'+
                '               </ul>'+
                '            </div>'+
                '            <div class="choonse_time_div">'+
                '              <div class="activeTime" id="tabTime_first"><ul></ul></div>'+
                '              <div id="tabTime_second"><ul></ul></div>'+
                '              <div id="tabTime_third"><ul></ul></div>'+
                '            </div>'+
                '       </div>'+
                '    </div>' +
            ...
            ,shade: 0.7, //不显示遮罩
            success: function(){
                if(Agency_Company_Info_type == '3'){
                    $('.login-inp .choose_time').hide();
                }
                if(House_Info_lan_co_id == '0' || House_Info_lan_co_id == '' || House_Info_lan_co_id == null){
                    $('.login-inp .choose_time').hide();
                }
                var mobile_renter=document.getElementById('mobile_renter');
                if(mobile_renter.value.length>0){
                    $("#mobile_renter").attr("readOnly","true");
                }
                getTime(0)
                // pop
                $(".choonse_time_content ul li:first-child").addClass('activeDay');
                $('#choonse_callback_time').on('click',function(){
                    $('.choonse_time_content').toggle();
                })
                var callback_value='';
                var callback_left='';
                var callback_right='';
                $('.choonse_time_content .choonse_time_content_day li').on('click',function(){
                    $(this).addClass("activeDay").siblings("li").removeClass("activeDay");
                    $(".choonse_time_div div ").eq($(this).index()).addClass("activeTime").siblings("div").removeClass("activeTime");
                    callback_left= callback_right=$(this).context.textContent;
                    console.log(callback_left);
                })     
                getHours();
                getHoursOtherDay()    
                
                $('.choonse_time_content .choonse_time_div li').on('click',function(){
                    $(this).addClass("activeDayRight").siblings("li").removeClass("activeDayRight");
                    if(callback_left==''){
                        callback_left=$('.choonse_time_content_day .activeDay').text();
                    }
                    callback_right=$(this).context.textContent;
                    $('.choonse_time_content').toggle();
                    callback_value=callback_left+'  '+callback_right;  
                    $('#choonse_callback_time').val(callback_value);
                })
...
function getTime(num){
    var time=new Date();
    time.setDate(time.getDate() +num);//num=0 表示当天
    ...
    var dateArr=[];
    var WeekNum = parseInt(week);//转数字
    var html='';
    var month_new='',day_new='';
    for(var i=num;i<3+num;i++){
        if(WeekNum==7){
            WeekNum=0; 
        }
        if(i==0){//当天
            time.setDate(time.getDate() );
            month_new=time.getMonth()+1;
            day_new=time.getDate();
            dateArr.push(month_new + "-" + day_new  + " (今天)")
        }else{
            if(i==1){//明天
                time.setDate(time.getDate() + 1);
                month_new=time.getMonth()+1;
                day_new=time.getDate();
                dateArr.push(month_new + "-" + day_new  + " (明天)")
            }else if(i==2){//后天
                time.setDate(time.getDate() +1);
                month_new=time.getMonth()+1;
                day_new=time.getDate();
                dateArr.push(month_new + "-" + day_new  + " (后天)")
            }
        } 
        WeekNum++; 
        day_new++;
    ...
    for(var j=0;j<dateArr.length;j++){
        html+='<li class="">'+dateArr[j]+'</li>'
    }
    $(".choonse_time_content .choonse_time_content_day ul").append(html);
    // activeDay
...
function getHours(){
    ...
    var TimeArr=[];
    var html='';
    html+='<li class="activeDayRight">尽快回电</li>'
    if( 7 <hours && hours< 20){
        for(var i=hours;i<22;i++){
            arr.push(i);
            TimeArr.push(i+':00-'+(i*1+1)+':00');
        }
        for(var j=0;j<TimeArr.length;j++){
            html+='<li class="">'+TimeArr[j]+'</li>'
        }
    }
    $("#tabTime_first ul").append(html);
    
   
}
function getHoursOtherDay(){
    var time=new Date();
    var hours=time.getHours()+1;     //小时
    var getMinutes=time.getMinutes();//分
    var arr=[];
    ...
    $("#tabTime_second ul").append(html);
    $("#tabTime_third ul").append(html);
