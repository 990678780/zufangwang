...
(function ($) {
            ...
            this.each(function () {
                ...
                if (args[0]) {
                    ...
                    if (args[0] == '*') {
                        ...
                        if (swfuploadify.queueData.files != null) {
                            for (var file in swfuploadify.queueData.files) {
                                swfuploadify.cancelUpload(file);
                            }
                            //for (var i = 0; i < swfuploadify.queueData.files.length; i++) {
                            //    swfuploadify.cancelUpload(swfuploadify.queueData.files[i].id);
                            //}
                        }
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '';//图片不能被添加:
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            ...
                            this.queueData.errorMsg += '\n一次最多上传 (' + settings.queueSizeLimit + ')张图片.';
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + file.name + '"超过了限制 (' + settings.fileSizeLimit + ').';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n文件"' + file.name + '" 为空.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + file.name + '"类型不正确，应该为 (' + settings.fileTypeDesc + ').';
                        break;
                }
            }
        ...
        onUploadSuccess: function (file, data, response, base64) {
            ...
            // Call the user-defined event handler
            if (settings.onUploadSuccess) settings.onUploadSuccess.call(this, file, data, response, base64);
        }
