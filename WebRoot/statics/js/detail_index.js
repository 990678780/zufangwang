...
function getOtherHouseItemHtml(item){
    var html = '<li num="'+item['house_id']+'" name="'+item['ga_name']+'" category="'+item['area_data']+'" variant="'+item['hire_way']+'" price="'+item['month_rent']+'">\
					<a onclick="pbalistshowkhouse(this)" target="_blank" href="'+item['link']+'?view_source=5" class="pro-pic"><img style="width: 100%" src="'+item['main_pic'] +'" /></a>\
					<div class="list-index-text">\
                    <div class="list-pic-title">\
					<h3>'+item['title'] +'</h3>\
					<span class="price"><em>￥</em> '+item['month_rent'] +'</span>\
				</div>\
				<p class="list-pic-ps">'+item['area_data']+item['desc'] +'</p>\
				<div class="pro-lab">';
...
function getrelationHouseItemHtml(item){
    var html = '<li num="'+item['house_id']+'" name="'+item['ga_name']+'" category="'+item['area_data']+'" variant="'+item['hire_way']+'" price="'+item['month_rent']+'">\
					<a onclick="pbalistshowkhouse(this)" target="_blank" href="'+item['link']+'?view_source=4" class="pro-pic"><img style="width: 100%" src="'+item['main_pic'] +'" /></a>\
					<div class="list-index-text">\
                    <div class="list-pic-title">\
					<h3>'+item['title'] +'</h3>\
					<span class="price"><em>￥</em> '+item['month_rent'] +'</span>\
...
function changeRelationHouse(){
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for( var i in data){
                var item = data[i];
                $('#relation_house_items_container').append( getrelationHouseItemHtml(item));
            }
...
var yuekanIng=0;
function yuekan(obj){
    var reg_trip=new RegExp("(^|&)view_source=([^&]*)(&|$)");
    if(window.location.href.indexOf('view_source')>0){
        var view_sourceCode=decodeURIComponent(window.location.search).substr(1).match(reg_trip)[2];
    }   
    ...
    var startDate=$('#choonse_callback_time').val();
    for(var i in paramsArr){
        if(paramsArr[i].name == 'verify_code' && renter_mobile!='')
            continue;
    ...
    _data += '&'+paramsArr[i].name+'='+paramsArr[i].value+'&view_source'+'='+view_sourceCode+'&start_date='+startDate;
    _hmt.push(['_trackEvent',g_config.from,g_config.city,'房源详情页提交预约看房']);
    ga('send', 'event',g_config.from,g_config.city,'房源详情页提交预约看房');
    yuekanIng=1;
    $.ajax({
